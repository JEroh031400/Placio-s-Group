...
$this->SetFont('times','B',9);
	...
	$stmnt=$db->query("SELECT accounts.Name,tr.BoatModel,tr.ServiceFee,Month(now()) FROM tr,accounts WHERE accounts.AccountID=tr.AccountID AND tr.StatusOfRepair='Finished already picked up' OR tr.StatusOfRepair='Finished ready for pick up'");
...
$this->SetFont('times','B',9);
	...
	$stmnt=$db->query("SELECT tr.StatusOfRepair,COUNT(tr.StatusOfRepair) AS 'Number',Month(now()) FROM tr,accounts WHERE tr.AccountID=accounts.AccountID GROUP BY tr.StatusOfRepair");
...
$pdf=new mypdf();
$pdf->title = 'Monthly Report of Repairs';

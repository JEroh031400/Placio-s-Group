...
$this->Ln();
	...
	$this->Cell(50,5,'BoatModel',1,0,'C');
	$this->Cell(40,5,'StatusOfRepair',1,0,'C');
	$this->Cell(40,5,'ServiceFee',1,0,'C');
...
$this->SetFont('times','B',9);
	...
	$stmnt=$db->query("SELECT accounts.Name,tr.BoatModel,tr.StatusOfRepair,tr.ServiceFee,year(now()) FROM tr,accounts WHERE accounts.AccountID=tr.AccountID");

     
$i=0;
	while($data=$stmnt->fetch(PDO::FETCH_OBJ)){
	$this->Cell(50,5,$data->Name,1,0,'L');
	$this->Cell(50,5,$data->BoatModel,1,0,'L');
	$this->Cell(40,5,$data->StatusOfRepair,1,0,'L');
	$this->Cell(40,5,$data->ServiceFee,1,0,'L');
	$this->Ln();
...
$this->SetFont('times','B',9);
	...
	$stmnt=$db->query("SELECT tr.StatusOfRepair,COUNT(tr.StatusOfRepair) AS 'Number',year(now()) FROM tr,accounts WHERE tr.AccountID=accounts.AccountID GROUP BY tr.StatusOfRepair");
...
$pdf=new mypdf();
$pdf->title = 'Yearly Report of Repairs';

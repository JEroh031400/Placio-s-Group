...
$this->Ln();
	$this->Cell(80,5,'Repair Description',1,0,'C');
	$this->Cell(50,5,'Client name',1,0,'C');
	
	$this->Cell(50,5,'Status Of Repair',1,0,'C');
...
$this->SetFont('times','B',9);
	...
	$stmnt=$db->query("SELECT * FROM tr,accounts WHERE tr.AccountID=accounts.AccountID");

	while($data=$stmnt->fetch(PDO::FETCH_OBJ)){
	$this->Cell(80,5,$data->Description,1,0,'L');
	$this->Cell(50,5,$data->Name,1,0,'L');
	$this->Cell(50,5,$data->StatusOfRepair,1,0,'L');
	$this->Ln();
}
...
$this->SetFont('times','B',9);
	...
	$stmnt=$db->query("SELECT tr.StatusOfRepair,COUNT(tr.StatusOfRepair) AS 'Number' FROM tr,accounts WHERE tr.AccountID=accounts.AccountID GROUP BY tr.StatusOfRepair");
